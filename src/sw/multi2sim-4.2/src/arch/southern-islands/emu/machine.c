	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		...
		for (i = 0; i < 2; i++)
		{
			si_isa_debug("S%u<=(%d)(%d,%gf) ", INST.sdst + i, 
	...
	struct si_inst_t *inst)
{
	...
	//si_isa_debug("sbase:%d offset:%d m_base:%d m_offset:%d\n", sbase, INST.offset, m_base, m_offset);
	...
	if (debug_status(si_isa_debug_category))
	{
		...
		for (i = 0; i < 4; i++)
		{
			si_isa_debug("S%u<=(%d)(%d,%gf) ", INST.sdst + i, m_base + m_offset + i * 4,
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		//si_isa_debug("Value of src0- %d:%x, src1- %d:%x\n", INST.ssrc0, s0.as_uint, INST.ssrc1, s1.as_uint);
		si_isa_debug("S%u<=(%d) ", INST.sdst, result.as_int, result.as_uint);
		si_isa_debug("scc<=(%d) ", nonzero.as_uint);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("S%u<=(%d) ", INST.sdst, s0_lo.as_uint);
		si_isa_debug("S%u<=(%d) ", INST.sdst + 1, s0_hi.as_uint);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("S%u<=(%d) ", INST.sdst, s0.as_uint);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	// if (debug_status(si_isa_debug_category))
	// {
	// 	si_isa_debug("S%u<=(%d) ", INST.sdst, pc_lo.as_uint);
	// 	si_isa_debug("S%u<=(%d) ", INST.sdst + 1, s0_hi.as_uint);
	// 	si_isa_debug("PC<=(%d) ", s0_lo.as_uint);
	// }
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		//si_isa_debug("S%d = %d, S%d = %d EXEC%d = %d, EXEC%d = %d\n", INST.ssrc0, s0_lo, INST.ssrc0 + 1, s0_hi,SI_EXEC, exec_lo, SI_EXEC + 1, exec_hi);
...
/* scc = (S0.i != S1.i). */
#define INST SI_INST_SOPC
void si_isa_S_CMP_LG_I32_impl(struct si_work_item_t *work_item,
	struct si_inst_t *inst)
{
	union si_reg_t s0;
	union si_reg_t s1;
	...
	struct si_inst_t *inst)
{
	...
	if (scc.as_uint)
	{
		...
		if (debug_status(si_isa_debug_category))
		{
//			si_isa_debug("wf%d: SCC=%u (taken) ", work_item->wavefront->id, scc.as_uint);
		}
	}
	else
	{
		...
		if (debug_status(si_isa_debug_category))
		{
//			si_isa_debug("wf%d: SCC=%u (not taken) ", work_item->wavefront->id, scc.as_uint);
		}
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (execz.as_uint)
	{
		...
		if (debug_status(si_isa_debug_category))
		{
	//		si_isa_debug("wf%d: EXEC=%d, EXECZ=%u (taken) ", work_item->wavefront->id, exec.as_uint, execz.as_uint);
		}
	}
	else
	{
		...
		if (debug_status(si_isa_debug_category))
		{
	//		si_isa_debug("wf%d: EXEC=%d, EXECZ=%u " "(not taken) ", work_item->wavefront->id, exec.as_uint, execz.as_uint);
		}
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	value.as_int = s0.as_int; // Hack for MIAOW
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V[%u]<=(%d) ", work_item->id, 
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V%u<=(%gf) ", work_item->id, 
			INST.vdst, product.as_float);
	}
}
#undef INST
...
/* D.i = S0.u[23:0] * S1.u[23:0]. */
#define INST SI_INST_VOP2
void si_isa_V_MUL_U32_U24_impl(struct si_work_item_t *work_item,
	struct si_inst_t *inst)
{
	union si_reg_t s0;
	union si_reg_t s1;
	...
	s0.as_uint = BITS32(s0.as_uint, 23, 0);
	s1.as_uint = BITS32(s1.as_uint, 23, 0);
	...
	struct si_inst_t *inst)
{
	...
	/* Calculate the result. */
	union si_reg_t r;
	r.as_float = s0.as_float * s1.as_float;   //MIAOW: A hack as, 3 port FPu is not present in MIAOW
	result.as_int = r.as_int + dst.as_int;
	...
	if (debug_status(si_isa_debug_category))
	{
		//si_isa_debug("s0: %d, s1: %d, s3: %d, s0 * s1: %d ", s0.as_int, s1.as_int, dst.as_int, r.as_int);
		si_isa_debug("t%d: V%u<=(%gf) ", work_item->id, INST.vdst,
			result.as_float);
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
	//	si_isa_debug("S%d:%f V%d:%f K:%f ",INST.src0, s0.as_float, INST.vsrc1, s1.as_float, K.as_float); 
		si_isa_debug("t%d: V%u<=(%gf) ", work_item->id, 
			INST.vdst, dst.as_float);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	sum.as_int = s0.as_int + s1.as_int;
	carry.as_uint = 
		!!(((unsigned long long) s0.as_uint + (long long) s1.as_uint) >> 32);
	...
	if (debug_status(si_isa_debug_category))
	{
	//	si_isa_debug("V%d: %d %d ", INST.src0, s0.as_uint, s0.as_int);
		si_isa_debug("t%d: V%u<=(%d) ", work_item->id,  
			INST.vdst, sum.as_int);
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("wf_id%d: vcc<=(%d) ",	work_item->id_in_wavefront, result.as_uint);
	}
}
#undef INST
...
/* D.i = S0.u[23:0] * S1.u[23:0]. */
#define INST SI_INST_VOP3a
void si_isa_V_MUL_U32_U24_VOP3a_impl(struct si_work_item_t *work_item,
	struct si_inst_t *inst)
{
	union si_reg_t s0;
	union si_reg_t s1;
...
*/
	...
	/* Calculate the result. */
	//prod.as_float = (s0.as_float * s1.as_float);
	//result.as_float = (s0.as_float * s1.as_float) + s2.as_float;

	union si_reg_t r;
	r.as_float = s0.as_float * s1.as_float;   //MIAOW: A hack as, 3 port FPu is not present in MIAOW
	result.as_int = r.as_int + s2.as_int;
	...
	if (debug_status(si_isa_debug_category))
	{
//		si_isa_debug("t%d = S0: (%d %gf), S1: (%d %gf), S2: (%d %gf), Prod: (%d %gf), Res: (%d %gf)\n", work_item->id, s0.as_int, s0.as_float, s1.as_int, s1.as_float, s2.as_int, s2.as_float, prod.as_int, prod.as_float,result.as_int, result.as_float);
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
	//	si_isa_debug("t%d: value:s0:%u, offset:s1:%u, width:s2:%u ", work_item->id, s0.as_uint, s1.as_uint, s2.as_uint);
	...
	struct si_inst_t *inst)
{
	...
	{
		result.as_int = (s0.as_int << (32 - s1.as_uint - s2.as_uint)) >> (32 - s2.as_uint);
	}
	else
	{
		result.as_int = s0.as_int >> s1.as_uint;
...
/* Calculate the minimum operand. */
	if (s0.as_int > s1.as_int)
	{
	   if(s0.as_int > s2.as_int)
			 max.as_int = s0.as_int;
	   else
			 max.as_int = s2.as_int;
	}
	else if(s1.as_int > s2.as_int)
	{
	   max.as_int = s1.as_int;
	}
	else
...
*/
	...
	/* Compare the operands. */
	result.as_uint = (s0.as_uint > s1.as_uint); //VIN- tweaked in to compare integer values instead of float to match RTL
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("wf_id%d: S[%u:+1]<=(%d) ",	work_item->id_in_wavefront, INST.vdst, result.as_uint);
		//si_isa_debug("wf_id%d: S[%u:+1]<=(%d) S0:(%u %gf) S1:(%u %gf)\n",	work_item->id_in_wavefront, INST.vdst, result.as_uint, s0.as_uint, s0.as_float, s1.as_uint, s1.as_float);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("wf_id%d: S[%u:%u],EXEC<=(%d) ",
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("wf_id%d: S[%d]<=(%d) ",
			work_item->id_in_wavefront, INST.vdst,
			result_lo.as_uint);
		si_isa_debug("wf_id%d: S[%d]<=(%d) ", work_item->id_in_wavefront, INST.vdst + 1,
			result_hi.as_uint);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("wf_id%d: S[%d:+1]<=(%lgf) ",
			work_item->id_in_wavefront, INST.vdst,
			value.as_double);
	}
}
	...
	struct si_inst_t *inst)
{
	...
	data0.as_uint = si_isa_read_vreg(work_item, INST.data0);
	m0.as_uint = si_isa_read_sreg(work_item, SI_M0);
...
*/
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("GDS?:%d DS[%d]<=(%d) ", INST.gds, 
			addr.as_uint, data0.as_uint);
	}
	else
	{
//		si_isa_debug("t%d: LDS[%u]<=(%u,%f) ", work_item->id,	addr.as_uint, data0.as_uint, data0.as_float);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	{
//		si_isa_debug("t%d: LDS[%u]<=(%d) ", work_item->id,addr.as_uint, data0.as_ubyte[0]);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	{
//		si_isa_debug("t%d: LDS[%u]<=(%d) ", work_item->id, addr.as_uint, data0.as_ushort[0]);
	}
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V%u<=(%d)(%d) ", work_item->id, 
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V%u<=(%d)(%d) ", work_item->id, 
			...
			data0.as_uint);
		si_isa_debug("V%u<=(%d)(%d) ", INST.vdst+1, 
	...
	struct si_inst_t *inst)
{
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V%d<=(%d) ", work_item->id, INST.vdst,
			data.as_int);
	}
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	if (INST.offen)
	{
		...
		if (INST.idxen)
			idx_vgpr = si_isa_read_vreg(work_item, INST.vaddr+1);
	}
	else
	{
		off_vgpr = INST.offset;
...
*/
	...
	//addr = base + mem_offset + inst_offset + off_vgpr + stride * (idx_vgpr + 0/*work_item->id_in_wavefront*/);

	//M2S-4.0 based addressing
	addr = off_vgpr;

	if (INST.idxen)
		addr += stride * ( idx_vgpr + work_item->id_in_wavefront);

	mem_read(si_emu->video_mem, addr, bytes_to_read, &value);
	si_isa_write_vreg(work_item, INST.vdata, value.as_uint);
	...
	if (debug_status(si_isa_debug_category))
	{
		si_isa_debug("t%d: V%u<=(%d)(%d,%gf) ", work_item->id, INST.vdata, addr, value.as_uint, value.as_float);
//		if (INST.offen) si_isa_debug("offen ");
//		if (INST.idxen)	si_isa_debug("idxen ");
//		si_isa_debug("%u,%u,%u,%u,%u,%u ", base, mem_offset, inst_offset, off_vgpr, idx_vgpr, stride);
	}
}
#undef INST
...
//	addr = base + mem_offset + inst_offset + off_vgpr + stride * (idx_vgpr + 0/*work_item->id_in_wavefront*/);
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (debug_status(si_isa_debug_category))
		{
			si_isa_debug("t%d: V%u<=(%d)(%d,%gf) ", work_item->id,
...
//	addr = base + mem_offset + inst_offset + off_vgpr +	stride * (idx_vgpr + work_item->id_in_wavefront);
	...
	if (debug_status(si_isa_debug_category))
	{
		...
		si_isa_debug("t%d: (%d)<=V%u(%gf) ", work_item->id, addr, INST.vdata, value.as_float);
		//si_isa_debug("t%d: (%d)<=V%u(%d,%gf) ", work_item->id, addr, INST.vdata, value.as_uint,value.as_float); //VIN - Modified to print only float values to match RTL
	}
}
#undef INST
...
*/
	...
	for (unsigned int i = 0; i < 2; i++)
	{
		...
		mem_write(si_emu->global_mem, addr, 4, &value);

		/* TODO Print value based on type */
		if (debug_status(si_isa_debug_category))
		{
			si_isa_debug("t%d: (%d)<=V%u(%d,%gf) ", work_item->id,
